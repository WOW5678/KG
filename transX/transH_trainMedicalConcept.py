...
class TransH(object):
    def __init__(self,entity2id,relation2id,tuples,margin=1,dim=150,batch_size=4,epochs=1000):
    ...
    ''''
        计算数据集中每个关系在生成负样例时遵从的概率值
        这里只记录tph/tph+hpt
        '''

    def calc_replace_prob(self):
        r_prob = {}
        rs = set([row[1] for row in self.tuples])
        print(self.tuples)
        for r in rs:
            print('r:', r)
            tph = {}
            ...
            for row in self.tuples:
                if row[1] == r:
                    if row[0] not in tph:
                        ...
                        tph[row[0]].append(row[2])
                    # 对每个尾实体计算头实体的个数
                    if row[2] not in hpt:
                        hpt[row[2]] = [row[0]]
                    elif row[2] in hpt:
                        hpt[row[2]].append(row[0])
            # 对tph的结果进行统计 求平均
            everageTph = len(tph.values()) / len(tph.keys())
            everageHpt = len(hpt.values()) / len(hpt.keys())
            r_prob[r] = everageTph / (everageTph + everageHpt)
        return r_prob

    def trainDataGenerate(self,r_prob):
        # 从训练数据集中进行取样
        samples=random.sample(self.tuples,self.batch_size)
        Tbatch=[]
        # 针对每个sample 生成它的负例元组
        for sample in samples:
            tuplewithCorruptedTuple=[sample,self.corruptedTupleGeneration(sample,r_prob)]
            Tbatch.append(tuplewithCorruptedTuple)
        return Tbatch
    def corruptedTupleGeneration(self,sample,r_prob):
        # 根据随机数随机选择替换头实体还是尾实体
        according_prob = r_prob.get(sample[1])
        i = np.random.uniform(-1, 1)
        if i<=2*according_prob:
            while True:
                #替换头实体
    ...
    def model_train(self,Tbatch,r_prob,train_batch_size=300):
        ...
        wr = tf.Variable(tf.random_uniform((len(r_prob), self.dim)), dtype=tf.float32)
        pos_embedding_tuples=tf.nn.embedding_lookup(embedding_table,pos_triple_batch)


        pos_embedding_tuples_norm=self.vector_norm(pos_embedding_tuples,wr)
        neg_embedding_tuples=tf.nn.embedding_lookup(embedding_table,neg_triple_batch)
        neg_embedding_tuples_norm=self.vector_norm(neg_embedding_tuples,wr)
    ...
    def vector_norm(self,e,wr):
        norm = tf.nn.l2_normalize(wr, 1)
